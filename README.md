...
## ⚠️ NHKは不採用（会員登録が必要）→ 時事通信/ITmediaに切り替え

当初 `sources.py` はNHK主要/経済のRSSを採用していましたが、実際にリンク先
を開くと記事本文の閲覧に「NHK ONE」への**無料会員登録が必須**になっている
ことが判明しました。見出し+リンクを配信するだけの本システムでは実用になら
ないため、NHKは外し、GitHub Actions run #3（2026-07-22）で以下に切り替えて
実際に生存確認・配信確認しています:

- **時事通信 人気ランキング** (`jiji.com/rss/ranking.rdf`, 主要ニュース代替):
  jiji.comトップページから自動発見。「ＮＨＫ映画、公開差し止め請求...」など
  社会・政治系も含む総合ニュースで、時事通信らしい幅広いカバレッジ
- **ITmedia ビジネスオンライン** (`business.xml`, 経済ニュース代替): 公式RSS
  利用条件ページで「私的利用は無償」と明記。会員登録なしで読める

run #3では実際にSlackへ11件配信できることを確認済みです。ただし
**RSSの生存確認だけでは「会員登録なしで本文が読めるか」までは判定できない**
ため、以下を必ず一度確認してください:

1. Slackに届いたリンクを実際に2〜3件開き、会員登録なしで本文が読めるか目視
   確認する（特に時事通信側）
2. 問題があれば `sources.py` の `SOURCES` を修正する（推測でURLを追加しない
   こと）

なお、当初候補にしていたITmedia NEWS(速報)/トップストーリー/国内ニュースは
生存確認できたものの、ITmedia NEWSの同一記事プールを返すだけで内容がほぼ
重複するため採用していません（`sources.py` にコメントで代替候補として記録
済み）。
