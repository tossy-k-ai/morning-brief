...
## ⚠️ NHKは不採用（会員登録が必要なため）／代替ソース検証中

当初 `sources.py` はNHK主要/経済のRSSを採用し、GitHub Actionsの
`workflow_dispatch`（run #1, #2, 2026-07-22実行）でRSS自体は生存確認済み
でした。しかし実際にリンク先を開くと、記事本文の閲覧に「NHK ONE」への
**無料会員登録が必須**になっていることが判明しました。見出し+リンクを配信
するだけの本システムでは実用にならないため、NHKは `sources.py` の
`SOURCES` から外しました。

代替として以下を候補に追加しています（**まだ生存確認できていません**）:

- **ITmedia NEWS / トップストーリー**: 公式RSS利用条件ページで
  「私的利用（RSSリーダー等での閲覧）は無償」と明記されており、記事は
  会員登録なしで読めます。ただしIT/ビジネス寄りのニュースで、政治・社会
  全般をカバーする「主要ニュース」の代わりとしては範囲が狭い点に注意
- **時事通信 (jiji.com)**: RSS提供のポリシーページはあるが具体的なフィード
  URLが特定できなかったため、`DISCOVERY_PAGES` にトップページとポリシー
  ページを追加し、`check_feeds.py --discover` で自動発見・確認する運用に
  しています

**本番運用の前に必ず以下を実行してください:**

1. Actionsタブから `Morning News Digest` を `workflow_dispatch`（手動実行）
   し、"Check RSS feed liveness (informational)" ステップと "Run digest"
   ステップの結果（Slackに届いた内容）を確認する
2. Slackに届いたリンクを実際に2〜3件開き、**会員登録なしで本文が読めるか**
   目視確認する（これはRSS自体の生存確認だけでは分からないため）
3. `[NG]` だったURLや、ログインが必要だったソースは `sources.py` の
...
- **RSS利用規約**: 使用するRSS提供元の利用規約を必ず確認してください。
  ...
  （例: Yahoo!ニュースRSSはプログラムでの利用・再配信を禁止。ITmediaは
  「私的利用は無償、商業利用は要許諾」としており、ニュース閲覧アプリ等での
  再配信は商用利用とみなされうる点に注意）。本システムは
