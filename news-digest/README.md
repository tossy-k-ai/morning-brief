...
このコードを書いた環境（クラウド上のLinuxサンドボックス）では、Google News・ITmediaの
RSSに実際にアクセスして生存確認する手段がありませんでした（組織のネットワークポリシーで
一般的な外部通信がブロックされていたため）。**本番運用の前に、実際にインターネットへ
アクセスできるあなたのPCで以下を必ず実行してください:**
...
- `config.yaml` の各ソースが `[OK]`（生存・パース可能）か確認してください
- Google News系のソースについては、`check_feeds.py` がリンクの実記事URLへの解決
  （リダイレクト追跡）も試みます。`[NG] Google Newsリンクを実記事URLに解決できません
  でした` と出た場合、Slack/Obsidianに届く記事リンクがGoogleの中間ページになる可能性が
  あります
- `[NG]` だったソースは `config.yaml` の `enabled: false` にするか、削除してください
  （推測でURLを書き換えないこと）

なお `ITmedia (速報)` / `ITmedia (AI+)` / `ITmedia ビジネスオンライン` は、同じ実装作業の
過程で親リポジトリ（morning-brief本体）の検証時に既に生存確認済みのURLをそのまま使って
います。Google News系の4本（主要/経済/ビジネス/AI）は今回新規に組んだクエリなので、
上記の確認が特に必要です。
